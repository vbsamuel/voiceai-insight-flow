...
const Index = () => {
  ...
  return <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 relative overflow-hidden">
      {/* Background Pattern */}
      <div className="absolute inset-0 bg-grid-slate-100 [mask-image:linear-gradient(0deg,white,rgba(255,255,255,0.6))] opacity-50" />
      
      <div className="relative z-10 flex h-screen bg-zinc-900">
        {/* Main Content Area */}
        <div className="flex-1 flex flex-col items-center justify-center p-8 bg-zinc-950">
          ...
          <div className="flex flex-col items-center space-y-8 max-w-4xl w-full">
            ...
            {/* Controls */}
            {currentState === 'idle' && <Button onClick={startListening} className="bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white px-8 py-3 rounded-full text-lg font-medium shadow-lg hover:shadow-xl transition-all duration-300">
                Start Listening
              </Button>}

            {currentState === 'results' && <Button onClick={resetToIdle} variant="outline" className="border-slate-300 text-slate-700 hover:bg-slate-50 px-6 py-2 rounded-full">
                Start New Search
              </Button>}

            {/* Website Icons */}
            {currentState === 'results' && <div className="w-full animate-fade-in">
                <WebsiteIcons />
              </div>}

            {/* Video Widget */}
            {currentState === 'results' && <div className="w-full max-w-2xl animate-fade-in">
                <VideoWidget />
              </div>}
          </div>
        </div>
